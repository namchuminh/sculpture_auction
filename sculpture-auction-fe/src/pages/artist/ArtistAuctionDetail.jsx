...
export default function ArtistAuctionDetail() {
  const { id } = useParams();
  ...
  async function closeAuction() {
    const res = await auctionsApi.close(id);
    await load();
  }
        ...
        <div className="rounded-2xl border bg-white p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button onClick={closeAuction} disabled={a.status !== "OPEN"}>
              {
                a.status !== "OPEN" ? "Phiên chưa mở" : "Đóng phiên (CLOSE)"
              }
          ...
          <div className="mt-2 text-xs text-slate-500">
              {
                a.status !== "OPEN" ? "Phiên chưa mở hoặc đã kết thúc." : "Chọn đóng phiên để kết thúc đấu giá sớm hơn thời gian dự kiến."
              }
          </div>
        </div>
